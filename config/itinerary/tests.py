...
class ItineraryEditViewTests(TestCase):
    ...
    def test_edit_requires_login(self):
        response = self.client.get(reverse("itinerary:edit", args=[self.itinerary.pk]))
    ...
    def test_edit_updates_without_regeneration(self):
        ...
        response = self.client.post(
            reverse("itinerary:edit", args=[self.itinerary.pk]),
            {
                "destination": "Paris",
        ...
        self.assertRedirects(response, reverse("itinerary:detail", args=[self.itinerary.pk]))
    ...
    @patch("itinerary.views.generate_itinerary", return_value=("New prompt", "Brand-new plan"))
...
class ItineraryCreateViewTests(TestCase):
    def setUp(self):
        self.url = reverse("itinerary:new")
    ...
    @patch("itinerary.views.generate_itinerary", return_value=("Prompt", "Plan body"))
    def test_preview_available_without_login(self, mock_generate, mock_events):
        response = self.client.post(
            self.url,
            {**self._valid_payload(), "action": "preview"},
        )
        self.assertRedirects(response, reverse("itinerary:preview"))
    ...
    @patch("itinerary.views.fetch_events", return_value=[])
    @patch("itinerary.views.generate_itinerary")
    ...
    @patch("itinerary.views.fetch_events", return_value=[])
    @patch("itinerary.views.generate_itinerary", return_value=("Prompt", "Saved plan"))
    def test_save_persists_for_authenticated_user(self, mock_generate, mock_events):
        ...
        self.assertRedirects(response, reverse("itinerary:detail", args=[itinerary.pk]))
    ...
    @patch("itinerary.views.generate_itinerary", return_value=("Prompt", "Plan preview"))
    def test_preview_then_save_after_login(self, mock_generate, mock_events):
        ...
        self.client.login(username="creator", password="StrongPass123!")
        response = self.client.post(reverse("itinerary:save_pending"))
        itinerary = Itinerary.objects.get(user=self.user)
        self.assertRedirects(response, reverse("itinerary:detail", args=[itinerary.pk]))
...
class ItineraryDeleteViewTests(TestCase):
    ...
    def test_delete_requires_login(self):
        response = self.client.get(reverse("itinerary:delete", args=[self.itinerary.pk]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_post_removes_itinerary(self):
        self.client.login(username="traveler", password="StrongPass123!")
        response = self.client.post(reverse("itinerary:delete", args=[self.itinerary.pk]))
